...
for (p in 1:valor) {
  ...
  dados[[p]] <- data.frame(
    sexo=sample(c(0,1), size = 100, replace = TRUE),
    idade=round(runif(100, 18, 80)),
    renda_sm=round(runif(100,500,30000))/1212
  )
 ...
 # cenario=(-8+10*dados[[p]]$sexo+
  ...
  dados[[p]]$y=factor(ifelse(prob>=quantile(prob,0.2),1,0))
...
boxplot_auc_regressao <- boxplot(as.numeric(auc_regressao),
                                 ...
                                 xlab = "AUC Regressão Logística", ylim = c(0.55,1),
