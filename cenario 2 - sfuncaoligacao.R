...
for (p in 1:valor) {
  ...
  dados[[p]] <- data.frame(
    ...
    y = factor(c(rep(1,600),rep(0,400))), # gerando 60% de inadimplentes e 40%
    # de adimplentes (pi=0.6), com n=100.
    
    sexo=c(sample(c(0,1),size=600, replace=TRUE, prob = c(0.4,0.6)), #40% dos inadimplentes são mulheres, 60% dos inadimplentes são homens
           sample(c(0,1),size=400, replace=TRUE, prob = c(0.7,0.3))), #70% dos adimplentes são mulheres, 30% dos adimplentes são homens
    #0 feminino, 1 masculino
    
    idade = round(c(
                  rtruncnorm(450, 18, 100, 25, 12), #dentro dos 60 inadimplentes,
                  # simulamos que cerca de 45 inadimplentes (3/4) tem idade menor,
                  # com média de 25 anos e desvio de 12
                  rtruncnorm(150, 18, 100, 70, 10), # também simulamos que cerca de
                  ...
                  rtruncnorm(400, 18, 100, 45, 10))), # 40 repetições de adimplentes, min 18 
    ...
    renda_sm = round(c(rtruncnorm(600, 500, 30000, 3000, 4000),
                       rtruncnorm(400, 500, 30000, 7000, 7000)))/1212
