...
for (p in 1:valor) {
  ...
  dados[[p]] <- data.frame(
    ...
    y = factor(c(rep(1,20),rep(0,80))), # gerando 20% de inadimplentes e 80%
    # de adimplentes (pi=0.2), com n=100.
    
    sexo=c(sample(c(0,1),size=20, replace=TRUE, prob = c(0.4,0.6)), #40% dos inadimplentes são mulheres, 60% dos inadimplentes são homens
           sample(c(0,1),size=80, replace=TRUE, prob = c(0.7,0.3))), #70% dos adimplentes são mulheres, 30% dos adimplentes são homens
    #0 feminino, 1 masculino
    
    idade = round(c(
                  rtruncnorm(15, 18, 100, 25, 12), #dentro dos 15 inadimplentes,
                  # simulamos que cerca de 45 inadimplentes (3/4) tem idade menor,
                  # com média de 25 anos e desvio de 12
                  rtruncnorm(5, 18, 100, 70, 10), # também simulamos que cerca de
                  #5 inadimplentes tem idade bastante alta,
                  #com média de 70 anos e desvio de 10. Ou seja, a relação da idade
                  #com a inadimplência é não linear.
                  rtruncnorm(80, 18, 100, 45, 10))), # 80 repetições de adimplentes, min 18 
    ...
    renda_sm = round(c(rtruncnorm(20, 500, 30000, 3000, 4000),
                       rtruncnorm(80, 500, 30000, 7000, 7000)))/1212
    # para a renda, entre os 20 inadimplentes, geramos uma renda com média 3000
    # e desvio 4000. Entre os 80 adimplentes, geramos uma renda com média 7000
...
boxplot_auc_regressao <- boxplot(as.numeric(auc_regressao),
                                 main = "Cenário 5: AUC Regressão para n = 100",
...
boxplot_auc_floresta <- boxplot(as.numeric(auc_floresta),
                                main = "Cenário 5: AUC Floresta para n = 100",
...
boxplot_auc_xgboost <- boxplot(as.numeric(auc_xgboost),
                               main = "Cenário 5: AUC XGBoost para n = 100",
                               xlab = "AUC XGBoost",
                               ylim = c(0.2,1),
...
boxplot_ks_regressao <- boxplot(as.numeric(test.ks_regressao),
                                main = "Cenário 5: KS Regressão para n = 100",
...
boxplot_ks_floresta <- boxplot(as.numeric(test.ks_floresta),
                               main = "Cenário 5: KS Floresta para n = 100",
...
boxplot_ks_xgboost <- boxplot(as.numeric(test.ks_xgboost),
                              main = "Cenário 5: KS XGBoost para n = 100",
                              xlab = "KS XGBoost",
                              ylim = c(0,1),
                              col = "lightgreen",pch=19,
                              horizontal = T,notch=F)
...
boxplot_ac_regressao <- boxplot(as.numeric(acuracia_logistica),
                                main = "Cenário 5: Acurácia Regressão para n = 100",
...
boxplot_ac_floresta <- boxplot(as.numeric(acuracia_floresta),
                               main = "Cenário 5: Acurácia Floresta para n = 100",
...
boxplot_ac_xgboost <- boxplot(as.numeric(acuracia_xgboost),
                              main = "Cenário 5: Acurácia XGBoost para n = 100",
